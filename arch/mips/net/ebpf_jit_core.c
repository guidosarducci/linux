...
static const struct {
	 ...
	 * the stack for storage, as needed on MIPS32.
	 */
...
void emit_const_to_reg(struct jit_ctx *ctx, int dst, u64 value)
{
...
static int build_int_prologue(struct jit_ctx *ctx)
{
	...
	if (tail_call_present(ctx) &&
	    !(ctx->flags & EBPF_TCC_IN_REG) && !tcc_sav)
	...
	if (tail_call_present(ctx) && !(ctx->flags & EBPF_TCC_IN_REG)) {
		if (tcc_sav)
			emit_instr(ctx, move, tcc_sav, tcc_reg);
		else
			emit_instr_long(ctx, sd, sw,
					tcc_reg, ctx->bpf_stack_off, MIPS_R_SP);
	}
...
void emit_bpf_call(struct jit_ctx *ctx, const struct bpf_insn *insn)
{
	...
	if ((insn->src_reg == BPF_PSEUDO_CALL) &&
	    tail_call_present(ctx) && !(ctx->flags & EBPF_TCC_IN_REG)) {
		/* Set TCC from reg or stack */
		if (tcc_sav)
			emit_instr(ctx, move, tcc_reg, tcc_sav);
		else
			emit_instr_long(ctx, ld, lw, tcc_reg,
...
int emit_bpf_tail_call(struct jit_ctx *ctx, int this_idx)
{
	...
	emit_instr_long(ctx, daddiu, addiu, tcc_reg, MIPS_R_T8, -1);
